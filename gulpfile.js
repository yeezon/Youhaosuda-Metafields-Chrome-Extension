...
var rootDir  = './',
    ...
    ignoreTypes = 'haml|styl|coffee|slim|jade',
    conf = {
      copy: {
...
gulp.task('haml', function() {
  gulp.src(conf.haml.src)
    .pipe(toys.plumber())
    .pipe(toys.cached(conf.haml.dist))
    .pipe(toys.haml())
    .pipe(gulp.dest(conf.haml.dist))
    .pipe(reload({stream: true}));
});
...
gulp.task('slim', function() {
  gulp.src(conf.slim.src)
    .pipe(toys.plumber())
    .pipe(toys.cached(conf.slim.dist))
    .pipe(toys.slim({pretty: true}))
    .pipe(gulp.dest(conf.slim.dist))
    .pipe(reload({stream: true}));
});
...
gulp.task('default', function(){
  gulp.start(['copy', 'haml', 'stylus', 'coffee', 'slim', 'jade', 'watch']);
});
